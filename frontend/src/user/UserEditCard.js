...
import { getCardById } from "../card/helper/cardapi";
import Base from "../core/Base";
import { updateCard } from "./helper/userapi";
...
const UserEditCard = () => {
  ...
  const onSubmit = (e) => {
    ...
    updateCard({ title, description, code, category, cardType }, cardId)
      .then((data) => {
        if (data.error) {
  ...
  const updateCardForm = () => {
    return (
      <div className="flex justify-center my-20 mx-5 md:mx-0">
        <div className="flex flex-col w-full max-w-md px-4 py-8 bg-white rounded-lg shadow-xl  dark:bg-gray-800 sm:px-6 md:px-8 lg:px-10">
  ...
  return <Base>{updateCardForm()}</Base>;
};
